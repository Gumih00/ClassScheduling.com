...
<form action="allotclasses.php" method="post" style="margin-top: 100px">
    ...
    <div align="center">
        <select name="course" class="list-group-item">
            ...
            ';
            <option value="2">BSIT 1st Year</option>
            <option value="3">BSIT 2nd Year</option>
            <option value="4">BSIT 3rd Year</option>
            <option value="5">BSIT 4th Year</option>
        </select>
    </div>
...
<div align="center">
    ...
    <table id=allotedclassroomstable>
        ...
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT * FROM classrooms ");
        $courses = array('BSIT 1st Year', 'BSIT 2nd Year', 'BSIT 3rd Year','BSIT 4th Year');
        while ($row = mysqli_fetch_assoc($q)) {
            if ($row['status'] == 0)
                continue;
